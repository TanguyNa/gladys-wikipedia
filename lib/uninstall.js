...
module.exports = function uninstall(){
	...
	//On pourrait réutiliser le user mais ne serait pas cohérent avec l'install
		...
		if(lang[0].language!='fr-FR')
			gladys.utils.sql('DELETE FROM sentence WHERE uuid in (?,?)', [sentences.sentenceWikipediaEn.uuid]);
		else gladys.utils.sql('DELETE FROM sentence WHERE uuid in (?,?)', [sentences.sentenceWikipediaFr.uuid]);
	});
